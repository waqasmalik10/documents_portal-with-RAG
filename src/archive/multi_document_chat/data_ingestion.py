...
class DocumentIngestor:
    ...
    def ingest_files(self, uploaded_files):
        try:
            ...
            for uploaded_file in uploaded_files:
                ...
                temp_path = self.session_temp_dir / unique_name
    ...
    def _create_retriever(self, documents):
        try:
            ...
            self.log.info("DOcument split into chunks")
            embeddings = self.model_loader.load_embeddings()
            
            vectorstore = FAISS.from_documents(documents=chunks, embedding=embeddings)
            vectorstore.save_local(str(self.session_faiss_dir))
            self.log.info("Vectorstore saved")

            retriever = vectorstore.as_retriever(search_type="similarity", search_params={"k": 10})
            return retriever
